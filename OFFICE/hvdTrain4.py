# correspond model4.py 
# add a threshold for selecting pseduo label
# 2018.10.25
...
def main():
    ...
    myTrainDataloader = Dataloader(args.img_dir, args.list1, args.list2,\
                              args.batch_size, args.h, args.w, args.num_threads)

    src_img = myTrainDataloader.simg_batch
    src_label = myTrainDataloader.slabel_batch
    tar_img = myTrainDataloader.timg_batch
    tar_label = myTrainDataloader.tlabel_batch
    ...
    if args.learning_strategy == 0:
        ...
        lr1 = baseLR1 / tf.pow(1 + 0.00045 * step_ph, 0.75)
        lr2 = baseLR2 / tf.pow(1 + 0.00045 * step_ph, 0.75)
    ...
    # alpha2=p2
    # threshold=0.9*adaptation_factor(step_ph/steps_per_epoch/5)
    threshold = tf.constant(0.,tf.float32)
    ...
    centroid_update_ops=model.build_losses(threshold,alpha1,alpha2,)
    model.build_outputs()
    ...
    with tf.control_dependencies([centroid_update_ops]):
        ...
        train_op_g = tf.group(train_op_1_1, train_op_1_2, 
                            train_op_2_1, train_op_2_2, 
                            bcast)

        train_op_d = tf.group(train_op_3_1, train_op_3_2,
                            bcast)
    ...
    for step in range(start_steps, num_steps):
        ...
        # _ =sess.run(train_op_d,feed_dict=feed_dict)
        # summary, total_loss, _ = sess.run(
        #     [summary_merged, model.F_loss, train_op_g], feed_dict=feed_dict)
        summary, total_loss, _ = sess.run(
