# DeepDICD v2.0
# vs v1.0
# utilizing moving centroid loss in
# another way
...
class DeepDICDModel(object):
    ...
    def build_summary(self):
        ...
        return [s1, s2, i1, i2, l1, l2, l3, l4, c1, c2, c3, c4, h1, h2, d1]
    ...
    def build_laplacian_regularization(self,centers_,features_,pred_):
        ...
        smooth=tf.nn.softmax_cross_entropy_with_logits(labels=gt,logits=pred)
        # smooth=tf.Print(smooth,[tf.reduce_min(smooth),tf.reduce_max(smooth),tf.shape(smooth)])
        loss=tf.reduce_mean(tf.multiply(distance,smooth))
