# 1. based on moving weight centorid
# 2. and adding label selection through a threshold
# 3. further, add a density estimation technique,
#    inspired by DIRT (adapt classifier, not feature)
# 4. change 'reuse' mehod using in encoder
# 2018.10.26
...
def toGreen(content): return termcolor.colored(
    content, "green", attrs=["bold"])
...
def toYellow(content): return termcolor.colored(
    content, "yellow", attrs=["bold"])


def toMagenta(content): return termcolor.colored(
    content, "magenta", attrs=["bold"])
...
class DeepDICDModel(object):
    ...
    def __init__(self, params, keep_prob, simg, slabel, timg, tlabel=None):
        ...
        self.source_moving_centroid = tf.get_variable(name='source_moving_centroid', shape=[
                                                      self.params.num_classes, 256], initializer=tf.zeros_initializer(), trainable=False)
    ...
    def build_model(self):
        with tf.variable_scope('model'):
            ...
            simg_adv = get_perturb_image(self.simg, self.slabel_pred, self.encoder)    
            _, self.slabel_pred_adv = self.encoder(simg_adv,reuse=True)
            
            self.tfc8, self.tlabel_pred = self.encoder(self.timg, reuse=True)
            timg_adv = get_perturb_image(self.timg, self.tlabel_pred, self.encoder)    
            _, self.tlabel_pred_adv = self.encoder(timg_adv,reuse=True)
    ...
    def encoder(self, x, reuse=False):
        with tf.variable_scope('source',reuse=reuse):
            fc8, score = self.build_alexnet(x)
        return fc8, score

    def build_losses(self, thrd, alpha1, alpha2, alpha3=10, beta1=1, beta2=1e-1):
        with tf.variable_scope('losses'):

            self.classify_loss_src = self.build_classify_losses(
                self.slabel, self.slabel_pred)
            self.classify_loss_tar = self.build_classify_losses(
                self.tlabel, self.tlabel_pred)
            self.build_adv_losses()

            # src_update,tar_update=self.build_centroid_based_losses()
            # src_update, tar_update = self.build_weights_centroid_based_losses()
            # src_update, tar_update = self.build_mask_weights_centroid_based_losses(thrd)
            src_update, tar_update = self.build_mask_centroid_based_losses(thrd)

            self.build_regular_losses()

            self.target_conditional_entropy_loss=self.build_conditional_entropy_losses(self.tlabel_pred,self.tlabel_pred)
            self.source_virtual_adversarial_loss=self.build_virtual_adversarial_losses(self.slabel_pred,self.slabel_pred_adv)
            self.target_virtual_adversarial_loss=self.build_virtual_adversarial_losses(self.tlabel_pred,self.tlabel_pred_adv)
            ...
            self.F_loss = self.classify_loss_src \
                ...
                + beta1  * (self.source_virtual_adversarial_loss) \
                + beta2  * (self.target_conditional_entropy_loss + self.target_virtual_adversarial_loss)
    ...
    def build_summary(self):
        ...
        l4 = tf.summary.scalar(
            'train/class_wise_adaptation_loss', self.class_wise_adaptation_loss)
        ...
        h1 = tf.summary.histogram(
            'train/source_moving_centroid', self.source_moving_centroid)
        h2 = tf.summary.histogram(
            'train/target_moving_centroid', self.target_moving_centroid)

        # d1 = tf.summary.scalar('train/tsmooth_loss',self.tsmooth)

        return [s1, s2, i1, i2, l1, l2, l3, l4, c1, c2, c3, c4, h1, h2]
        # return [s1, s2, i1, i2, l1, l2, l3, l4, c1, c2, c3, c4, d1]
    ...
    def build_alexnet(self, x):
        ...
        # 8th Layer: FC and return unscaled activations (for
        # tf.nn.softmax_cross_entropy_with_logits)
        fc8 = fc(fc7, 4096, 256, relu=False, name='fc8')
        score = fc(fc8, 256, self.params.num_classes,
    ...
    def build_classify_losses(self, gt, pred):
        ...
        # Chong: vs tf.loss.sparse_softmax_cross_entropy(labels,logits)
        loss = tf.reduce_mean(
    ...
    def build_wadv_losses(self):

        alpha = tf.random_uniform([tf.shape(self.tfc8)[0], 1], 0., 1.)
        diff = self.sfc8 - self.tfc8
        interpolates = self.tfc8 + alpha * diff

        with tf.variable_scope('domain_descriminate') as DD:
            target_logits, _ = Domain(self.tfc8)
            tgradients = tf.gradients(target_logits, [self.tfc8])[0]
        with tf.variable_scope(DD, reuse=True):
            source_logits, _ = Domain(self.sfc8)
            sgradients = tf.gradients(source_logits, [self.sfc8])[0]
        with tf.variable_scope(DD, reuse=True):
            interp_logits, _ = Domain(interpolates)
            igradients = tf.gradients(interp_logits, [interpolates])[0]

        # gradients=tf.concat([tgradients,sgradients,igradients],0)
        gradients = igradients
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), 1))
        gradients_penalty = tf.reduce_mean((slopes - 1.)**2)

        wd_loss = tf.reduce_mean(source_logits) - tf.reduce_mean(interp_logits)
        D_loss = wd_loss + gradients_penalty * 10.

        G_loss = wd_loss

        self.G_loss = 0.3 * G_loss
        self.D_loss = 0.3 * D_loss

    def build_regular_losses(self):
        domain_var = [v for v in tf.trainable_variables()
                      if 'domain' in v.name]
        generator_var = [v for v in tf.trainable_variables()
                         if 'domain' not in v.name]
        ...
        self.dregular_loss = 0.0005 * \
            tf.reduce_mean([tf.nn.l2_loss(v)
                            for v in domain_var if 'weights' in v.name])
        self.gregular_loss = 0.0005 * \
            tf.reduce_mean([tf.nn.l2_loss(v)
                            for v in generator_var if 'weights' in v.name])
    ...
    def build_mask_centroid_based_losses(self, thrd):

        target_softmax_ = tf.nn.softmax(self.tlabel_pred, axis=1)
        source_softmax_ = tf.nn.softmax(self.slabel_pred, axis=1)
        target_mask = tf.reduce_max(target_softmax_, 1) > thrd
        source_mask = tf.reduce_max(source_softmax_, 1) > thrd

        source_label = tf.boolean_mask(self.slabel, source_mask)
        target_label = tf.boolean_mask(
            tf.argmax(self.tlabel_pred, 1), target_mask)
        source_feature = tf.boolean_mask(self.sfc8, source_mask)
        target_feature = tf.boolean_mask(self.tfc8, target_mask)

        # target_label = tf.Print(target_label,[tf.shape(target_label)])
        # source_label = tf.Print(source_label,[tf.shape(source_label)])

        sones = tf.ones_like(source_feature)
        tones = tf.ones_like(target_feature)
        current_source_count = tf.unsorted_segment_sum(
            sones, source_label, self.params.num_classes)
        current_target_count = tf.unsorted_segment_sum(
            tones, target_label, self.params.num_classes)
        ...
        current_source_centroid = tf.divide(tf.unsorted_segment_sum(
            data=source_feature, segment_ids=source_label, num_segments=self.params.num_classes), current_positive_source_count)
        current_target_centroid = tf.divide(tf.unsorted_segment_sum(
            data=target_feature, segment_ids=target_label, num_segments=self.params.num_classes), current_positive_target_count)
        ...
        self.class_wise_adaptation_loss = self.build_class_wise_adaptation_losses(
            source_moving_centroid, target_moving_centroid)
        self.sintra_loss = self.build_batch_intra_losses(
            source_moving_centroid, source_feature, source_label)
        self.tintra_loss = self.build_batch_intra_losses(
            source_moving_centroid, target_feature, target_label)

        self.sinter_loss = self.build_batch_inter_losses(
            source_moving_centroid, source_feature, source_label)
        self.tinter_loss = self.build_batch_inter_losses(
            target_moving_centroid, target_feature, target_label)
    ...
    def build_mask_weights_centroid_based_losses(self, thrd):
        ...
        target_softmax = tf.boolean_mask(target_softmax_, target_mask)
        source_softmax = tf.boolean_mask(source_softmax_, source_mask)
        target_feature = tf.boolean_mask(self.tfc8, target_mask)
        source_feature = tf.boolean_mask(self.sfc8, source_mask)
        source_label = tf.boolean_mask(self.slabel, source_mask)
        target_label = tf.boolean_mask(tf.argmax(self.tlabel_pred,-1), target_mask)
        
        target_softmax_expand = tf.expand_dims(target_softmax, -1)
        target_feature_expand = tf.expand_dims(target_feature, 1)
        ...
        current_positive_source_count = tf.maximum(
            current_source_count, tf.ones_like(current_source_count))
        current_positive_target_count = tf.matmul(
            current_target_count, tf.ones([1, tf.shape(target_feature)[1]], tf.float32), True)

        current_source_centroid = tf.divide(tf.unsorted_segment_sum(
            data=source_feature, segment_ids=source_label, num_segments=self.params.num_classes), current_positive_source_count)
        current_target_centroid = tf.divide(
            tf.reduce_sum(tf.matmul(target_softmax_expand, target_feature_expand), axis=0), current_positive_target_count)
    ...
    def build_batch_inter_losses(self, centers, features, labels):
        ...
        current_count = tf.unsorted_segment_sum(
            ones, labels, self.params.num_classes)
        current_positive_count = tf.maximum(
            current_count, tf.ones_like(current_count))
        current_centroid = tf.divide(tf.unsorted_segment_sum(
            data=features, segment_ids=labels, num_segments=self.params.num_classes), current_positive_count)
        ...
        current_whole_centroid_roll = tf.manip.roll(
            current_whole_centroid, shift=rnd, axis=0)

        loss = tf.reduce_mean(
            tf.square(current_batch_centroid - current_whole_centroid_roll))
        return loss
    ...
    def build_laplacian_regularization(self, centers_, features_, pred_):
        pred_ = tf.nn.softmax(pred_)
        # pred_ = tf.Print(pred_,[pred_[0]])
        centers = tf.nn.l2_normalize(centers_, 1)
        features = tf.nn.l2_normalize(features_, 1)
        distance = tf.matmul(centers, features, False, True)
        gt_ = tf.one_hot(tf.cast(tf.linspace(
            0., 30., self.params.num_classes), tf.int64), self.params.num_classes)
        gt = tf.tile(tf.expand_dims(gt_, 1), [1, tf.shape(features)[0], 1])
        pred = tf.tile(tf.expand_dims(pred_, 0), [tf.shape(centers)[0], 1, 1])
        smooth = tf.reduce_sum(tf.square(pred - gt), 2)
        loss = 2 * tf.reduce_sum(tf.multiply(distance, smooth))
        # pdb.set_trace()
    ...
    def build_conditional_entropy_losses(self,gt,pred):
        
        loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(logits=pred,labels=tf.nn.softmax(gt)))
        return loss

    def build_virtual_adversarial_losses(self,pred,pred_adv):

        loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(logits=pred_adv, labels=tf.nn.softmax(pred)))
        return loss
