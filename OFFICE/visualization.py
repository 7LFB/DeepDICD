...
def main():
    ...
    for line in f.readlines():
        ...
        feed_dict={image:img}
        features_,pred_=sess.run([model.sfc8,model.slabel_pred],feed_dict=feed_dict)
        pred_=np.mean(pred_,0)
        features=np.mean(features_,0)
    ...
    X=np.array(X)
    y=np.array(y)
    fig_name=args.model_name + '-' + args.snapshot_dir.split('/')[-1] 
    visualization_using_tsne(X,y,1,count,'./output/'+fig_name+'.png')
    
    print(toGreen('save in '+'./output/'+fig_name+'.png'))
