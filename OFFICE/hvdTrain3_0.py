...
def main():
    ...
    trainDataloader = Dataloader(args.img_dir, args.list1, args.list2,
                              args.batch_size, args.h, args.w, args.num_threads)
    valDataloader = testDataloader(args.img_dir, args.list1, args.list2,
                              1, args.h, args.w, args.num_threads)

    train_val=tf.placeholder(tf.bool,name='train_val_phase')

    train_src_img = trainDataloader.simg_batch
    train_src_label = trainDataloader.slabel_batch
    train_tar_img = trainDataloader.timg_batch
    train_tar_label = trainDataloader.tlabel_batch

    val_src_img = valDataloader.simg_batch
    val_src_label = valDataloader.slabel_batch
    val_tar_img = valDataloader.timg_batch
    val_tar_label = valDataloader.tlabel_batch

    src_img=tf.cond(train_val,lambda:train_src_img,lambda:val_src_img)
    src_label=tf.cond(train_val,lambda:train_src_label,lambda:val_src_label)
    tar_img=tf.cond(train_val,lambda:train_tar_img,lambda:val_tar_img)
    tar_label=tf.cond(train_val,lambda:train_tar_label,lambda:val_tar_label)
    ...
    if args.learning_strategy == 0:
        ...
        lr1 = baseLR1 / tf.pow(1 + 0.0007 * step_ph, 0.85)
        lr2 = baseLR2 / tf.pow(1 + 0.0007 * step_ph, 0.85)
    ...
    # alpha3 = tf.constant(0.,tf.float32)
    # alpha2=p2
    ...
    for step in range(start_steps, num_steps):
        ...
        feed_dict = {step_ph: step, keep_prob: 0.5, train_val:True}
